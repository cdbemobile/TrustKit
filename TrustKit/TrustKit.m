...
{
	[sharedTrustKit updateConfiguration:trustKitConfig];
}
...
{
	if (!trustKitConfig || trustKitConfig.count <= 0) {
		return;
	}
	NSDictionary *newConfig = parseTrustKitConfiguration(trustKitConfig);
	BOOL wasSwizzled = [_configuration[kTSKSwizzleNetworkDelegates] boolValue];
	BOOL willBeSwizzled = [newConfig[kTSKSwizzleNetworkDelegates] boolValue];
	if(wasSwizzled && !willBeSwizzled) {
		...
		[NSException raise:@"TrustKit configuration invalid"
					format:@"Cannot disable swizzling configuration in a TrustKit sharedInstance"];
	}
	_configuration = parseTrustKitConfiguration(trustKitConfig);
	[self setupPinningValidator];
}
